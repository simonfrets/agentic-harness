...
loaded after the bundles in `.harness/rules/`, which are the `builtin` layer
that `harness init --update` manages.
...
Files here are never overwritten by `harness init --update`.
